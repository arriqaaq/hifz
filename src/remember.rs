...
///
/// Embeds richer text (title + content + concepts + files) so semantic search
/// works against saved memories, not just raw observations. Then runs Phase-3
/// deterministic link generation (KNN + concept + file Jaccard) to populate
/// `mem_link` edges between the new memory and existing neighbours.
pub async fn save(
    db: &Surreal<Db>,
    embedder: &Embedder,
    project: &str,
    ...
    if let Some(new_id) = created.into_iter().next().and_then(|c| c.id) {
        if let Err(e) =
            link::generate_links(db, &new_id, project, &embedding, concepts, files).await
        {
        ...
        for ent in entities::extract(files, concepts, &body) {
            let _ = entities::upsert(db, ent.kind, &ent.name, project).await;
        }
        if let Err(e) = link_by_shared_entities(db, &new_id, project, concepts, files).await {
...
/// (file or concept) with. `via='entity'` with score ∝ shared count.
async fn link_by_shared_entities(
    ...
    if concepts.is_empty() && files.is_empty() {
        return Ok(());
    }

    // Find memories in the same project that share at least one file or concept.
    #[derive(Debug, SurrealValue)]
    struct Row {
        id: Option<RecordId>,
    ...
    let mut resp = db
        .query(
            "SELECT id, concepts, files FROM hifz \
    ...
    let self_set: std::collections::HashSet<&str> = concepts
        .iter()
        .chain(files.iter())
        .map(String::as_str)
        .collect();
    ...
    for r in rows {
        ...
        let score = shared as f64 / total as f64; // Jaccard over concepts ∪ files
...
/// Phase 1a: richer-text input — title + content + concepts + files.
