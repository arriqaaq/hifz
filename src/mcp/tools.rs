...
fn tool_defs() -> Vec<serde_json::Value> {
    vec![
        ...
        serde_json::json!({"name": "hifz_evolve", "description": "Run Memory Evolution on a memory — LLM refines neighbour tags/context/links (requires HIFZ_LLM_EVOLVE=true and Ollama)", "inputSchema": {"type": "object", "properties": {"memory_id": {"type": "string", "description": "RecordId like 'memory:xyz'"}}, "required": ["memory_id"]}}),
        ...
        // Typed graph + project surface tools.
        ...
        serde_json::json!({"name": "hifz_warmup", "description": "Build the project-scoped warmup digest — latest plan, recent decisions, conventions, open bugs, gotchas, failure patterns, recent lessons. Inject the `top` field as system context to summarise project state at session start.", "inputSchema": {"type": "object", "properties": {"session_id": {"type": "string"}, "project": {"type": "string", "description": "Defaults to the session's project."}, "top_n": {"type": "integer", "default": 15}}, "required": ["session_id"]}}),
